...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!isWithinWorkingHours(business.working_hours)) {
      await sendWhatsAppMessage(fromPhone, toPhone, business.out_of_hours_message);
      await saveConversation(business.id, customerPhone, customerName, messageContent, business.out_of_hours_message, "out_of_hours", 1.0);
    ...
    await sendWhatsAppMessage(fromPhone, toPhone, finalResponse);
    
    // Save conversation
    await saveConversation(business.id, customerPhone, customerName, messageContent, finalResponse, responseType, confidence);
